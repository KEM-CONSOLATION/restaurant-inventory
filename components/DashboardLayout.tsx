...
import { checkSetupStatus } from '@/lib/utils/setup-check'
...
export default function DashboardLayout({ children, user }: DashboardLayoutProps) {
  ...
  useEffect(() => {
    const checkSetupAndTour = async () => {
      // Only check if tour hasn't been manually completed
      if (hasCompletedTour()) {
        setCheckingSetup(false)
        return
      }

      // For superadmins, check tour completion only
      ...
      // For other users, check if setup is incomplete
      if (user.organization_id) {
        try {
          const setupStatus = await checkSetupStatus(user.organization_id, user.role as string)

          // Only run tour if setup is incomplete
      ...
      items: [
        {
          ...
          roles: ['branch_manager', 'admin', 'tenant_admin'],
        },
        {
          name: 'Restocking',
    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          roles: ['admin', 'tenant_admin', 'branch_manager'],
        },
      ],
    },
  ]
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <nav className="flex-1 px-4 py-6 space-y-6 overflow-y-auto" data-tour="sidebar">
            {navGroups.map(group => {
